...
local function radarRows()
  ...
  local rows = {
    Sec("Датчик"),
    Info("Статус", function()
      if not rd.available() then return "датчик не найден" end
      return "найден: " .. U.shortAddr(rd.address and rd.address() or "?") .. (rd.lastErr and (" · ошибка: " .. U.trunc(rd.lastErr, 30)) or "")
    end, rd.available() and (rd.lastErr and C.warn or C.ok) or C.bad),
    Info("Игроков рядом сейчас", function() return tostring(#rd.list()) end),
...
local function systemRows()
  ...
  rows[#rows + 1] = Btn("Диагностика устройств → Журнал", function()
    local types = {
      "draconic_reactor", "flux_gate", "draconic_rf_storage",
      "me_interface", "me_controller", "ae2_interface",
      "sensor", "radar", "openperipheral_sensor",
      "chat_box", "openperipheral_bridge", "glasses_bridge", "terminal_glasses_bridge", "glasses", "terminal",
    }
    local log = P.log
    log.info("ДИАГНОСТИКА", "── что видит компьютер прямо сейчас ──")
    for _, t in ipairs(types) do
      local list = {}
      for a in component.list(t) do list[#list + 1] = a end
      if #list > 0 then
        local shown = {}
        for i = 1, math.min(3, #list) do shown[#shown + 1] = U.shortAddr(list[i]) .. "…" end
        log.info("ДИАГНОСТИКА", t .. ": найдено " .. #list .. " — " .. table.concat(shown, ", "))
      else
        log.warn("ДИАГНОСТИКА", t .. ": ничего не найдено (0)")
      end
    end
    local R = P.mods.reactor
    log.info("ДИАГНОСТИКА", "реакторов в списке PRISMA: " .. R.count() .. " / " .. R.max())
    for i, r in ipairs(R.list()) do
      log.info("ДИАГНОСТИКА", string.format("  #%d: онлайн=%s, реактор=%s, вход=%s, выход=%s",
        i, tostring(r.online), U.shortAddr(r.rAddr) .. "…", U.shortAddr(r.inAddr) .. "…", U.shortAddr(r.outAddr) .. "…"))
    end
    UI.toast("Готово — смотрите вкладку Журнал", "ok")
  end, "нужно для разбора «устройство не видно»", nil)
...
local PAGES = {
  ...
  { id = "reactors", title = "Реакторы", rows = function()
    return {
      ...
      N("Начало форс-режима, °C", { "reactors", "forceModeTemp" }, 2000, 9000, 100, { int = true, big = 500, hint = "ниже этой темп. — минимальный поток, быстрый разгон" }),
      N("Целевая темп. удержания, °C", { "reactors", "targetTemp" }, 2000, 9500, 100, { int = true, big = 500, hint = "плавный выход на неё и удержание" }),
      N("Безопасная темп. (для меток), °C", { "reactors", "safeModeTemp" }, 2000, 9500, 100, { int = true, big = 500 }),
  ...
  { id = "chat", title = "Чат", rows = function()
    return {
      Sec("Чат (chat_box, игровой канал)"),
      B("Команды !prisma", { "chat", "commands" }, "status / craft / reactors / sing"),
      N("Хранить строк", { "chat", "maxLines" }, 10, 300, 10, { int = true, hint = "буфер истории, не влияет на очки" }),
      Sec("Окно чата в очках"),
      Info("Где настроить", function() return "размер и ширина окна чата в очках — на вкладке «ОЧКИ»" end, C.dim),
    }
  end },
