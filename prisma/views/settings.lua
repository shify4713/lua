...
local ROLES = {
  ...
  { key = "slotReader", label = "Чтение слота ME", types = { "inventory_controller", "transposer" } },
}
...
local PAGES = {
  { id = "general", title = "Общие", rows = function()
    return {
      ...
      N("Сторона слота ME (-1=авто)", { "me", "slotSide" }, -1, 5, 1, { int = true, hint = "сторона Adapter: down/up/back/front/right/left = 0..5" }),
  ...
  { id = "reactors", title = "Реакторы", rows = function()
    return {
      ...
      N("Целевая темп., °C", { "reactors", "targetTemp" }, 5000, 8500, 50, { int = true, big = 100, hint = "к ней выравниваем после разгона (~7.8k)" }),
      N("Начало разгона до, °C", { "reactors", "forceModeTemp" }, 2000, 9000, 100, { int = true, big = 500 }),
      ...
      N("Потолок разгона, RF/t", { "reactors", "boostFlow" }, 1e5, 5e7, 50000, { big = 200000, fmt = unumf, int = true }),
      N("Шаг разгона, RF/t", { "reactors", "boostStep" }, 1000, 50000, 1000, { big = 5000, fmt = unumf, int = true }),
