...
function X.addReactorManual()
  ...
  local function gateLabel(g)
    local f = g.flow or 0
    local tag = f > 0 and ("  поток " .. U.num(f)) or "  поток 0"
    if g.signal and g.actual and g.signal ~= g.actual then
      tag = "  лимит " .. U.num(g.signal) .. " / факт " .. U.num(g.actual)
    end
    return U.shortAddr(g.addr) .. "…" .. tag
  end
  modal().pick({
...
function X.craftFromSlot()
  local spec, err = P.me.configSlot()
  if not spec then
    UI.toast(err or "не удалось прочитать слот", "warn")
    return
  end
  -- если уже есть — просто выберем; иначе форма настроек
  local existing = P.mods.autocraft.find(spec.name, spec.damage)
  if existing then
    P.state.sel = existing
    UI.toast("Уже в списке: " .. existing.label, "info")
    return
  end
  craftNew(spec)
  if spec.reader then
