...
local function drawChat(x, y, w, h)
  ...
  UI.panel(x, y, w, h, "ЧАТ", { right = Ch.available() and nil or "нет chat_box (только приём)", rcolor = C.dim })
  local ix, iw = x + 2, w - 4
  local logs = Ch.logs()
  local rows = h - 2
  local wrapped = V.chatWrap(logs, iw, 40)
  local from = math.max(1, #wrapped - rows + 1)
  for i = from, #wrapped do
    local e = wrapped[i]
    local ry = y + 1 + (i - from)
    if not e.cont then UI.text(ix, ry, e.head, e.color, C.panel) end
    UI.text(ix + e.headLen, ry, e.text, e.cont and C.dim or C.text, C.panel)
  end
end
