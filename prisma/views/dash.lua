...
local function drawChat(x, y, w, h)
  ...
  local logs = Ch.logs()
  local n = h - 2
  -- длинные сообщения переносим на несколько строк вместо обрезки «…»; разворачиваем
  -- последние сообщения (с запасом), потом берём последние n получившихся строк
  local flat = {}
  local from = math.max(1, #logs - 20)
  for k = from, #logs do
    local e = logs[k]
    local nameShown = U.trunc(e.name, 10)
    local nx = math.min(11, U.ulen(nameShown) + 1)
    for li, txt in ipairs(U.wrap(e.msg, math.max(4, iw - nx))) do
      flat[#flat + 1] = { nameShown = nameShown, nx = nx, first = (li == 1), text = txt }
    end
  end
  local shown = {}
  for i = math.max(1, #flat - n + 1), #flat do shown[#shown + 1] = flat[i] end
  for i, ln in ipairs(shown) do
    local ry = y + h - #shown - 1 + i
    if ln.first then UI.text(ix, ry, ln.nameShown, C.acc, C.panel) end
    UI.text(ix + ln.nx, ry, ln.text, C.text, C.panel)
  end
end
...
local function drawEvents(x, y, w, h)
  ...
  local ix, iw = x + 2, w - 4
  local n = h - 2
  -- то же самое: переносим по словам, показываем последние n получившихся строк
  local list = P.log.recent(30)
  local flat = {}
  for _, e in ipairs(list) do
    for li, txt in ipairs(U.wrap(e.msg, math.max(4, iw - 6))) do
      flat[#flat + 1] = { e = e, first = (li == 1), text = txt }
    end
  end
  local shown = {}
  for i = math.max(1, #flat - n + 1), #flat do shown[#shown + 1] = flat[i] end
  for i, ln in ipairs(shown) do
    local ry = y + h - #shown - 1 + i
    if ln.first then UI.text(ix, ry, string.sub(ln.e.clock, 1, 5), C.dim, C.panel) end
    UI.text(ix + 6, ry, ln.text, C[LVL[ln.e.lvl]] or C.text, C.panel)
  end
end
