...
local function drawDash()
  ...
  local mods = {
    ...
    {"m_c", "core", "Energy Core"},
    {"m_p", "precraft", "Precraft"},
    {"m_s", "singularity", "Singular"},
    {"m_g", "glasses", "Glasses AR"},
    {"m_rad", "radar", "Radar"},
    {"m_ch", "chat", "Local Chat"},
  }
  ...
  if #pls == 0 then
    ui.text(w-33, 6, "no contacts", ui.C.DIM)
  else
    for i, n in ipairs(pls) do
      if i > 8 then break end
      ui.text(w-33, 5+i, "> " .. unicode.sub(n, 1, 28), ui.C.ALERT)
    end
  end
