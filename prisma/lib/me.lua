...
-- ───────── количество предмета ─────────
-- разные мосты ждут в фильтре getItemsInNetwork/getCraftables разные поля
-- (name/id, damage/dmg) — подтверждено вашим рабочим скриптом «прекрафт», где перебираются
-- именно эти 4 варианта. Пробуем их по очереди, пока фильтр не будет принят (ok == true).
local function filterVariations(name, dmg)
  return {
...
function M.rawQty(name, dmg)
  ...
  for _, filter in ipairs(filterVariations(name, dmg)) do
    ...
    if ok and type(res) == "table" then
      ...
      for i = 1, (res.n or #res) do
        ...
        if type(s) == "table" then
          local sid = s.id or s.name
          if sid == name then q = q + (tonumber(s.size) or tonumber(s.qty) or tonumber(s.count) or 0) end
        end
      end
      return q
    end
...
function M.request(name, dmg, amount, cpuName)
  ...
  local cr, lastErr
  for _, filter in ipairs(filterVariations(name, dmg)) do
    local ok, res = pcall(dev.getCraftables, filter)
    if ok and type(res) == "table" and (res.n or #res) > 0 then
      cr = res
      break
    end
    lastErr = ok and "нет рецепта" or res
  end
  if not cr then
    if type(lastErr) == "string" and lastErr ~= "нет рецепта" then markFail(lastErr) else markOk() end
    return nil, (lastErr == "нет рецепта" and "нет рецепта" or ("ME: " .. U.trunc(tostring(lastErr), 40)))
  end
  markOk()
  local c = cr[1]
  local ok2, h, err
...
-- ПРИМЕЧАНИЕ: раньше здесь был M.listNetwork() — полный список всей ME-сети одним вызовом
-- getItemsInNetwork() без фильтра. На большой сети (тысячи разных предметов) это создаёт
-- огромную Lua-таблицу и валит компьютер по нехватке памяти (out of memory). Функцию убрали
-- целиком вместе с пунктом меню «Из сети ME» — добавление предмета теперь только вручную
-- (ID + damage) или чтением физического слота интерфейса (см. M.slotItem ниже).

-- предмет из физического слота ME-интерфейса (что реально лежит в слоте, а не что
-- настроено как паттерн экспорта — конфиг-слот часто пуст, даже если предмет лежит в слоте)
function M.slotItem(slot)
  if not dev then return nil, "нет ME" end
  slot = slot or 1
  -- сперва — реальное содержимое слота; конфиг-паттерн (если он вообще поддерживается
  -- этим мостом) — только как запасной вариант, если сам слот прочитать не удалось
  for _, fn in ipairs({ "getStackInSlot", "getInterfaceConfiguration" }) do
    if type(dev[fn]) == "function" then
      local ok, s = pcall(dev[fn], slot)
      if ok and type(s) == "table" then
        -- ВАЖНО: у разных мостов ID предмета лежит в разных полях — где-то .name,
        -- где-то .id (подтверждено вашим рабочим скриптом: stack.id or stack.name),
        -- damage тоже бывает и .damage, и .dmg. Проверяем оба варианта каждого.
        local id = s.id or s.name
        local dmg = s.damage or s.dmg or 0
        if id then
          return { name = id, damage = dmg, label = s.label or s.name or id }
        end
      end
    end
  end
  return nil, "слот " .. slot .. " пуст или не читается"
end
M.configSlot = M.slotItem   -- старое имя — для обратной совместимости
