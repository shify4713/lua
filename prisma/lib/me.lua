...
-- предмет из конфиг-слота интерфейса: перебираем несколько слотов и несколько
-- вариантов имени метода, т.к. они отличаются между версиями AE2/мостов OC
function M.configSlot(slot)
  if not dev then return nil, "нет ME" end
  local slots = slot and { slot } or { 1, 2, 3, 4, 5, 6, 7, 8, 9 }
  local fns = { "getInterfaceConfiguration", "getStackInSlot", "getConfiguration", "getInterfacePattern" }
  ...
  return nil, "не найден настроенный предмет в слотах 1-9 интерфейса"
end
