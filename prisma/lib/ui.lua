-- PRISMA CORE UI v3 — Zero flicker, modern, fast
...
M.C = {
  ...
  INPUT=0x0C1420, SELECT=0x0A2A3A
}
...
function M.init()
  ...
  w, h = math.min(160, mw), math.min(50, mh)
...
function M.frame(x,y,ww,hh,title)
  ...
  for i=1,hh-2 do gpu.set(x,y+i,"│"); gpu.set(x+ww-1,y+i,"│") end
...
function M.progress(x,y,ww,pct,fg)
  ...
  gpu.setBackground(0x1A2530); gpu.fill(x,y,ww,1," ")
...
function M.header(title,right)
  gpu.setBackground(M.C.PANEL); gpu.fill(1,1,w,1," ")
...
function M.drawTabs(tabs,active)
  ...
  gpu.setForeground(M.C.FRAME); gpu.setBackground(M.C.BG)
  gpu.set(1,3,string.rep("─",w))
end

-- Модальный ввод (не мерцает основной экран)
function M.input(title,prompt,def)
  ...
  local str = def or ""
  local function ri()
    gpu.setBackground(M.C.INPUT); gpu.fill(ix,my+2,maxl,1," ")
    gpu.setForeground(M.C.ACCENT); gpu.set(ix,my+2,unicode.sub(str.."_",1,maxl))
  end
  ri()
  ...
  while true do
    ...
    if ev=="key_down" then
      ...
      if a2==14 then if #str>0 then str=unicode.sub(str,1,-2) end; ri()
