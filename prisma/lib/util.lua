...
-- перенос текста по словам на несколько строк шириной не больше width (в символах).
-- возвращает массив строк (минимум одна, даже для пустого текста). Слово длиннее width
-- разрезается принудительно, чтобы не вылезти за границу панели.
function U.wrap(s, width)
  s = tostring(s or "")
  width = math.max(1, math.floor(width or 40))
  local out = {}
  if ulen(s) <= width then return { s } end
  local cur = ""
  for word in s:gmatch("%S+") do
    while ulen(word) > width do
      local head = usub(word, 1, width)
    ...
    if cur == "" then
      cur = word
    elseif ulen(cur) + 1 + ulen(word) <= width then
      cur = cur .. " " .. word
    else
      out[#out + 1] = cur
      cur = word
...
function U.compList(t)
  local r = {}
  -- ВАЖНО: без третьего аргумента "true" (точное совпадение типа). У некоторых модов/сборок
  -- реальное имя типа компонента не совпадает СИМВОЛ В СИМВОЛ с ожидаемым (например с версией
  -- аддона), из-за чего "точный" поиск не находит вообще ничего, хотя устройство подключено.
  -- Подстрочный поиск (как в рабочих скриптах) находит его в любом случае.
  for a in component.list(t) do r[#r + 1] = a end
  table.sort(r)
  return r
end
