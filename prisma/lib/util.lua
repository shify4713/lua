...
-- реальное время сервера (не требует интернет-карты): computer.realTime() отдаёт
-- системные часы Java-процесса сервера в мс, это НЕ время игрового дня Minecraft.
-- os.date/os.time внутри песочницы OpenOS завязаны на игровые тики и «зацикливаются»
-- каждые ~20 реальных минут (1 игровые сутки), поэтому для настоящего времени суток
-- используем именно computer.realTime().
function U.clock()
  ...
  elseif mode == "game" then
    ...
    if ok and type(ms) == "number" then
      ...
      local sec = math.floor(ms / 1000) + tz * 3600
      local okd, s = pcall(os.date, "!%H:%M:%S", sec)
      if okd and s then return s end
    end
  end
  return U.dur(U.now())
end
