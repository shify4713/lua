...
local function build()
  ...
  if g.show.reactors and mods.reactor and mods.reactor.count() > 0 then
    ...
    for i, rr in ipairs(r.list()) do
      if rr.online then
        ...
        text("r" .. i .. ".t", pad, y, string.format("R%.0f %.0f°  %.0f%%", i, math.floor(d.temp), math.floor(d.field * 100)), col)
  ...
  if g.show.craft and mods.autocraft then
    ...
    if #list > 0 then
      ...
      for i, it in ipairs(list) do
        ...
        frac = frac or 0
        local nameMax = math.max(6, math.floor((W - pad * 2) / cw * 0.5))
  ...
  if g.show.near and mods.radar then
    ...
    if #pl > 0 then
      ...
      text("nr.t", pad, y, "РЯДОМ: " .. U.trunc(names, math.floor((W - pad * 2) / cw) - 8), COL.bad)
      y = y + lh
    end
  end
  ...
  -- чат (теперь рисуется ПЕРВЫМ, сверху; остальная инфа — под ним)
  -- длинные сообщения переносим по словам на несколько строк вместо обрезки «…»;
  -- g.chatLines теперь означает высоту в строках ЭКРАНА, а не число сообщений —
  -- берём сообщений с запасом, разворачиваем и оставляем последние chatLines строк
  local chat = {}
  if g.show.chat and mods.chat and g.chatLines > 0 then
    local raw = mods.chat.lines(math.max(g.chatLines, 20))
    local flat = {}
    for _, line in ipairs(raw) do
      for _, txt in ipairs(U.wrap(line, g.chatWidth)) do flat[#flat + 1] = txt end
    end
    for i = math.max(1, #flat - g.chatLines + 1), #flat do chat[#chat + 1] = flat[i] end
  end
  local chatH = 0
  if #chat > 0 then
    chatH = pad + lh + #chat * (lh - 1) + pad - 1
  end
  local chatGap = chatH > 0 and 3 or 0
  local mainY = chatH + chatGap   -- на столько сдвигаем вниз основной блок (ядро/реакторы/крафт/...)

  -- координаты панели по якорю
  local totalH = mainH + mainY
  local ox, oy = g.x, g.y
  ...
  if chatH > 0 then
    out[#out + 1] = { id = "cbg", kind = "box", x = 0, y = 0, w = W, h = chatH, color = COL.bg, alpha = g.alpha }
    out[#out + 1] = { id = "cbg.l", kind = "box", x = 0, y = 0, w = W, h = 1, color = COL.acc, alpha = 0.85 }
    local ty = pad - 1
    out[#out + 1] = { id = "ch.h", kind = "text", x = pad + 2, y = ty, text = "ЧАТ", color = COL.acc, scale = s }
    ty = ty + lh
    for i, line in ipairs(chat) do
      out[#out + 1] = { id = "ch" .. i, kind = "text", x = pad + 2, y = ty, text = line, color = COL.text, scale = s * 0.95 }
      ty = ty + lh - 1
    end
  end

  -- фон и акцентная линия основного блока — перед текстом (рисуются под ним)
  out[#out + 1] = { id = "bg", kind = "box", x = 0, y = mainY, w = W, h = mainH, color = COL.bg, alpha = g.alpha }
  out[#out + 1] = { id = "bg.l", kind = "box", x = 0, y = mainY, w = W, h = 1, color = COL.acc, alpha = 0.85 }
