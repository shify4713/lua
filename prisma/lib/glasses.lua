...
local function build()
  ...
  -- фон и акцентная линия — в начало списка (рисуются под текстом)
  local out = {}
  out[#out + 1] = { id = "bg", kind = "box", x = 0, y = 0, w = W, h = mainH, color = COL.bg, alpha = g.alpha }
  out[#out + 1] = { id = "bg.l", kind = "box", x = 0, y = 0, w = W, h = 1, color = COL.acc, alpha = 0.85 }
  for _, e in ipairs(els) do out[#out + 1] = e end

  if chatH > 0 then
    local cy = mainH + 3
    out[#out + 1] = { id = "cbg", kind = "box", x = 0, y = cy, w = W, h = chatH, color = COL.bg, alpha = g.alpha }
    out[#out + 1] = { id = "cbg.l", kind = "box", x = 0, y = cy, w = 1, h = chatH, color = COL.acc, alpha = 0.7 }
    local ty = cy + pad - 1
