...
-- разные аддоны называют/реализуют датчик игроков по-разному —
-- пробуем все известные варианты типа компонента
local SENSOR_TYPES = { "sensor", "radar", "openperipheral_sensor", "player_detector", "entity_sensor" }
-- и разные имена метода получения списка игроков
local METHODS = { "getPlayers", "getPlayerNames", "players", "list" }
...
function R.init()
  ...
  R.lastErr = nil
  local p, a = U.findComp(SENSOR_TYPES, P.cfg.devices.sensor)
  sensor, sensorAddr = p, a
  if not sensor then
    for _, t in ipairs(SENSOR_TYPES) do
      local addr = component.list(t)()
      if addr then sensor, sensorAddr = U.proxy(addr), addr; break end
    end
  end
  if sensor then
    warnedMissing = false
    log.info("РАДАР", "датчик найден: " .. U.shortAddr(sensorAddr))
  elseif not warnedMissing then
    warnedMissing = true
    log.warn("РАДАР", "датчик игроков не найден (нужен sensor / openperipheral_sensor)")
  end
end
...
-- находит первый рабочий метод получения списка на этом датчике (кешируется)
local function findMethod()
  if not sensor then return nil end
  for _, m in ipairs(METHODS) do
    if type(sensor[m]) == "function" then return m end
  end
end
...
function R.update(now)
  ...
  if not cachedMethod or type(sensor[cachedMethod]) ~= "function" then cachedMethod = findMethod() end
  if not cachedMethod then
    if R.lastErr ~= "нет подходящего метода на датчике" then
      R.lastErr = "нет подходящего метода на датчике"
      log.warn("РАДАР", "у датчика нет ни одного из известных методов (" .. table.concat(METHODS, "/") .. ")")
    end
    nearby = {}
    return
  end
  local ok, res = pcall(sensor[cachedMethod])
  if not ok then
    if R.lastErr ~= tostring(res) then
      R.lastErr = tostring(res)
      log.warn("РАДАР", cachedMethod .. "(): " .. U.trunc(tostring(res), 60))
    end
    return
  end
  R.lastErr = nil
