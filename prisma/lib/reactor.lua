...
local function step(r, idx, now)
  ...
  if not ok or type(inf) ~= "table" then
    r.fail = r.fail + 1
    r.lastErr = ok and "getReactorInfo вернул не таблицу" or tostring(inf)
    if r.fail == 3 then
      log.warn("РЕАКТОР " .. idx, "нет ответа от реактора: " .. U.trunc(r.lastErr, 70))
    end
    if r.fail >= 3 then
      r.online = false
      r.stateText = "НЕТ СВЯЗИ"
  ...
  -- выходной гейт: разгон → плавный подход к целевой температуре → удержание
  -- (низкий поток = быстрый рост температуры; поток ≈ генерации = температура стоит на месте —
  --  поэтому греем на минимальном потоке, а по мере приближения к цели плавно поднимаем поток
  --  до уровня генерации, чтобы не проскочить цель по инерции)
  local out = r.lastOut or 0
  local hold = U.clamp(c.targetTemp or c.safeModeTemp, c.forceModeTemp + 100, c.tempCrit - 50)
  if not r.auto then
  ...
  elseif kind == "running" then
    ...
    elseif temp < hold then
      r.stateText = "ПОДХОД"
      local span = math.max(1, hold - c.forceModeTemp)
      local frac = U.clamp((temp - c.forceModeTemp) / span, 0, 1)
      local desired = c.initialFlow + frac * math.max(0, gen - c.initialFlow)
      if math.abs(out - desired) > 1500 then gateWrite(r, "out", desired) end
    else
      r.stateText = "ДЕРЖУ " .. string.format("%.0f°", hold)
