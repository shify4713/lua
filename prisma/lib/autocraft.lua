...
local SAVE_FIELDS = { "name", "damage", "label", "keep", "batch", "trigger", "prio", "enabled", "cooldown", "timeout", "once", "note", "total", "runs" }
-- goal НЕ сохраняется: для «разового» крафта цель всегда пересчитывается заново при каждом запуске программы
...
function A.apply(it, v)
  ...
  normalize(it)
  it.rt.goal = nil         -- цель для «once» пересчитается заново
  it.rt.nextScan = 0
  it.rt.nextTry = 0        -- новые настройки применяются сразу, без ожидания старого кулдауна
  it.rt.cool = 0
  it.rt.fails = 0
  markDirty()
end
...
function A.importSlot()
  local s, err = me.configSlot(1)
...
local function target(it)
  if it.once then return it.rt.goal or it.keep end
  return it.keep
end
A.target = target
...
local function onStock(it, q, err)
  ...
  rt.have = q
  rt.haveAt = now
  if it.once and not rt.goal then rt.goal = q + it.keep end
  local tgt = target(it)
