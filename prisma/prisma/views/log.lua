...
local function drawEvents(x, y, w, h)
  ...
  local rows = h - 4
  local cw = w - 2 - 1
  local wrapped = V.logWrap(list, cw, 14)
  local first, last = UI.scrollArea("log.events", x + 1, y + 3, w - 2, rows, #wrapped, true)
  for i = first, last do
    local e = wrapped[i]
    ...
    UI.text(x + 2 + e.prefixLen, ry, e.text, C[LCOL[e.lvl]] or C.text, C.panel)
  end
  if #wrapped == 0 then UI.textC(x + 1, y + 5, w - 2, "записей нет", C.dim, C.panel) end
end

local function drawChat(x, y, w, h)
  local Ch = P.mods.chat
  local status = Ch.available() and "chat_box: приём и отправка" or "только приём (нет chat_box для отправки)"
  UI.panel(x, y, w, h, "ЧАТ", { right = status, rcolor = Ch.available() and C.dim or C.warn })
  local ix, iw = x + 2, w - 4
  ...
  local rows = h - 5
  local cw = w - 2 - 1
  local wrapped = V.chatWrap(logs, cw, 200)
  local first, last = UI.scrollArea("log.chat", x + 1, y + 3, w - 2, rows, #wrapped, true)
  for i = first, last do
    local e = wrapped[i]
    local ry = y + 3 + (i - first)
    if not e.cont then UI.text(x + 2, ry, e.head, e.color, C.panel) end
    UI.text(x + 2 + e.headLen, ry, e.text, e.cont and C.dim or C.text, C.panel)
  end
