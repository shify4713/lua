...
function S.draw(x, y, w, h)
  ...
  local c = { idx = 3, need = 8, have = 10, bar = 16, can = 7, sing = 8, ord = 10 }
  local fixed = c.idx + c.need + c.have + c.bar + c.can + c.sing + c.ord + 8
  c.name = math.max(12, cw - 2 - fixed)
  local xs, cx = {}, lx + 2
  for _, k in ipairs({ "idx", "name", "need", "have", "bar", "can", "sing", "ord" }) do xs[k] = cx; cx = cx + c[k] + 1 end
  ...
  head("bar", "Прогресс"); head("can", "Можно", true); head("sing", "В МЭ", true); head("ord", "Заказ", true)
  ...
  for i = first, last do
    ...
    UI.text(xs.sing, ry, U.pad(U.int(info.sing), c.sing, true), info.sing > 0 and C.acc or C.dim, bg)
    local oi = M.orderInfo(it)
